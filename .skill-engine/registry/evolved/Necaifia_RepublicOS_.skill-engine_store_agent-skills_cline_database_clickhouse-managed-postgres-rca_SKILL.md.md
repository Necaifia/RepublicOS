---
description: MUST USE when investigating performance issues on a ClickHouse-managed
  Postgres instance. Provides an evidence-based RCA workflow that scrapes the Prometheus
  endpoint for system signal, pulls per-digest evidence from the Slow Query Patterns
  API, and recommends (does not apply) a fix. Uses transactional safety with compensating
  actions, connection pooling, and query validation.
license: Apache-2.0
metadata:
  author: ClickHouse Inc
  evolved: true
  evolved_at: '2026-07-20T14:07:05.338053+00:00'
  version: 0.1.0
