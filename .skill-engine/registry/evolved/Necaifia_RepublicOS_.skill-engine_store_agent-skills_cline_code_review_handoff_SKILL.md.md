---
description: Create a structured handoff document at context/handoff.md at the end
  of a session so the next agent can onboard without prior context. Uses git to discover
  modified files, filters out ephemeral subagent work artifacts under context/, enforces
  a strict format, and validates the result with a lint script. The handoff is the
  single source of truth for the next session. Use whenever a session ends, a task
  is paused, or work is being passed to another agent. Applies structured code review
...
metadata:
  evolved: true
  evolved_at: '2026-07-20T14:07:05.237238+00:00'
name: handoff
tags:
- documentation
- code_review
