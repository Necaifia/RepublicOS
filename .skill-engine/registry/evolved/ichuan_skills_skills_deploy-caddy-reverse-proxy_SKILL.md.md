---
description: 'Deploy Caddy reverse proxy on remote servers with automatic SSL and
  systemd integration. Use when users want to: (1) Set up reverse proxy for local
  web services, (2) Configure automatic Let''s Encrypt SSL certificates, (3) Set up
  systemd service with auto-start, (4) Proxy HTTP/WebSocket traffic. Triggered by
  phrases like ''deploy caddy'', ''setup reverse proxy'', ''configure caddy'', ''caddy
  ssl''. Follows agent-native CI/CD with eval-gated promotion, canary rollouts, and
  compensating rollback actions.'
metadata:
  evolved: true
  evolved_at: '2026-07-20T14:07:04.703455+00:00'
