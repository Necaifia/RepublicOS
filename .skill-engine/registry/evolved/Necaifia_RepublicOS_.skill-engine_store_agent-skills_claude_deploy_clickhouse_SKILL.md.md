---
description: Connect to and query ClickHouse (a local server or a ClickHouse Cloud
  service) from the terminal using the official clickhousectl CLI, including the browser
  OAuth login flow. Use when the user wants to run SQL against ClickHouse, explore
  schemas and tables, inspect Cloud services, or authenticate clickhousectl. For building
  a local dev environment or deploying to Cloud, defer to the official ClickHouse
  skills (see Scope). Follows agent-native CI/CD with eval-gated promotion, canary
  rollouts, and compensating rollback actions.
metadata:
  evolved: true
  evolved_at: '2026-07-20T14:07:04.888173+00:00'
name: clickhouse
tags:
- infrastructure
- security
